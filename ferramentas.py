...
def informacoes_dataframe(pergunta: str, df: pd.DataFrame) -> str:
    ...
    columns = df.dtypes
    nulos = df.isnull().sum()
    nans_str = df.apply(lambda col: col[~col.isna()].astype(
        str).str.strip().str.lower().eq('nan').sum())
...
def resumo_estatistico(pergunta: str, df: pd.DataFrame) -> str:
    ...
    estatisticas_descritivas = df.describe(
        include='number').transpose().to_string()
  ...
  # Template otimizado para geração de código de gráficos
    template_resposta = PromptTemplate(
        template="""
            ...
            1. Utilize SEMPRE o DataFrame completo chamado `df`.
            2. Se a solicitação envolver:
                - "média por"
                - "tempo médio"
                - "valor médio"
                - "média agrupada"
                - outros similares
                então você DEVE:
                    a) criar explicitamente um DataFrame agregado usando
                    df_agg = df.groupby(<coluna_categorica>, as_index=False)[<coluna_numerica>].mean()
                    b) usar df_agg no gráfico
            3. NÃO use amostras (head, sample, iloc).
            4. Use as bibliotecas `matplotlib.pyplot` (como `plt`) e `seaborn` (como `sns`).
            5. Defina o tema com `sns.set_theme()`
            6. Certifique-se de que todas as colunas mencionadas na solicitação existem no DataFrame chamado `df`.
            7. Escolha o tipo de gráfico adequado conforme a análise solicitada:
            ...
            8. Configure o tamanho do gráfico com `figsize=(8, 4)`.
            9. Adicione título e rótulos (`labels`) apropriados aos eixos.
            10. Posicione o título à esquerda com `loc='left'`, deixe o `pad=20` e use `fontsize=14`.
            11. Formate os ticks do eixo X de forma que favoreça a visualização do usuário. 
            12. Remova as bordas superior e direita do gráfico com `sns.despine()`.
            13. Finalize o código com `plt.show()`.
...
def criar_ferramentas(df):
    ferramenta_informacoes_dataframe = Tool(
        ...
        func=lambda pergunta: informacoes_dataframe.run(
            {"pergunta": pergunta, "df": df}),
    ...
    ferramenta_resumo_estatistico = Tool(
        ...
        func=lambda pergunta: resumo_estatistico.run(
            {"pergunta": pergunta, "df": df}),
